...
import { useCreateSessionMutation, useGetQrCodeMutation } from "@/app/Redux/features/sessionApiSlice";
...
const Page = () => {
  ...
  const [changeTime, setChangeTime] = useState({ changeSpeed: 7, timeWorking: 30 });

  // Create session
  const handleCreateSession = async () => {
    try {
      const response = await createSession({
        course: selectedCourse || "67b8f87450147de33624ea4d",
  ...
  return (
    <div className="takeAttendance mt-7 flex flex-col items-center gap-10 w-full pb-8">
      <div className="actions w-full flex flex-row items-center justify-evenly">
        <Link className="bg-[#FDD05B] py-3 px-5 font-inder text-lg flex gap-3 items-center" href="#">
          <FaRegAddressCard /> Show Students
        </Link>
        <Link className="bg-[#FDD05B] py-3 px-5 font-inder text-lg flex gap-3 items-center" href="/dashboard/doctor/manualAttendance">
          <FaUserEdit /> Manual Attendance
        </Link>
        <button className="bg-[#FDD05B] py-3 px-5 font-inder text-lg flex gap-3 items-center" onClick={handleCreateSession}>
      ...
      <div className="setQR w-[40rem] flex flex-col items-start gap-2">
        ...
        <div className="materials w-full bg-[#FEEFC7] rounded-lg flex items-center justify-between gap-5 p-4"
          style={{ boxShadow: "-2px 2px 8px 0px rgba(116,116,116,0.75)", border: "1px solid" }}>
      ...
      <div className="materials w-4/5 min-h-80 bg-[#F6F6F6] rounded-lg flex justify-center items-center p-4"
        style={{ boxShadow: "-2px 2px 8px 0px rgba(116,116,116,0.75)", border: "1px solid" }}>
        {qrCode && (
          <Image src={qrCode} alt="QR Code" className="w-82 h-82" width={300} height={300} />
        )}
      </div>

      <Image src="/dash.svg" alt="dash" width={100} height={100} className="absolute bottom-0 right-0 w-[20rem] -z-10" />
    </div>
  );
};
